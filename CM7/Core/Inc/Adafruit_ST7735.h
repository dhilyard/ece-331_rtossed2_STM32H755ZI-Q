...
struct st7735_cmd {
	const uint8_t cmd;	// The command to send
	const uint8_t cnt;	// data count
	const uint16_t delay;	// Delay - if no delay is needed, set to zero.  in msecs.
	const uint8_t data[16];	// Passed data arguments
};
